...
''' 
    image_data=cv2.imread('a.png')
    cv2.imwrite('a.png', image_data
'''
...
def load_images(folder):
    ...
    for filename in os.listdir(os.path.join(root_folder, folder)):
        ...
        #count = 1
        img = cv2.imread(os.path.join(root_folder, folder, filename), 0)
        if img is not None:
            images.append(img)
...
for folder in folders: #get all images and labels into 2 seperate lists
